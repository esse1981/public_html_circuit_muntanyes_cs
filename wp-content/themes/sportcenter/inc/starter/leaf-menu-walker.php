<?php
class leafcolor_walker_nav_menu extends Walker_Nav_Menu {
	...
	// add main/sub classes to li's and links
	 function start_el( &$output, $item, $depth = 0, $args = array(),  $current_object_id = 0) {
		...
		if(count($icon_classes) > 0){
			$leaf_icon_names = '<i class="'.esc_attr( implode( '"></i><i class="', $icon_classes ) ).'"></i>';
		}
		global $leaf_columns;
		if($depth==0){
			$leaf_columns = false;
		...
		$item_output = sprintf( '%1$s<a%2$s'.($is_parent&&$depth==0?' data-toggle="dropdown"':'').'>'.$leaf_icon_names.'%3$s%4$s%5$s ' . ($item->description&&$depth==0?'<span class="menu-description">'.$item->description.'</span>':'') .'</a>%6$s',
...
class leafcolor_walker_nav_menu_mobile extends Walker_Nav_Menu {
