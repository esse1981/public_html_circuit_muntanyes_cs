...
global $wl_options;
if((!$wl_options = get_option('sportcenter')) || !is_array($wl_options) ) $wl_options = array();
...
function leafcolor_expand_control(){
	...
	update_option('sportcenter', $wl_options);
}
...
function leafcolor_widget_add_custom_fields() {
	...
	//var_dump(get_option('sportcenter'));
	...
	echo "<p><label for='".esc_attr($id_disp)."-leafcolor'>".__('Custom Variation', 'sportcenter').": <input class='widefat' type='text' name='".esc_attr($id_disp)."-leafcolor' id='".esc_attr($id_disp)."-leafcolor' value='".esc_attr($value)."' /></label></p>";
}
...
if ( is_admin() )
{
    add_action( 'sidebar_admin_setup', 'leafcolor_widget_style_expand_control' );
}

function leafcolor_widget_style_expand_control()
{   
    ...
    {   // controll-less widgets need an empty function so the callback function is called.
        if (!$wp_registered_widget_controls[$id])
            wp_register_widget_control($id,$widget['name'], 'leafcolor_widget_style_empty_control');
        $wp_registered_widget_controls[$id]['callback_style_redirect'] = $wp_registered_widget_controls[$id]['callback'];
        $wp_registered_widget_controls[$id]['callback'] = 'leafcolor_widget_style_extra_control';
        array_push( $wp_registered_widget_controls[$id]['params'], $id );   
    }
	// UPDATE CUSTOM FIELDS OPTIONS (via accessibility mode?)
	if ( 'post' == strtolower($_SERVER['REQUEST_METHOD']) )
...
// added to widget functionality in 'leafcolor_widget_style_expand_control' (above)
function leafcolor_widget_style_empty_control() {}

// added to widget functionality in 'leafcolor_widget_style_expand_control' (above)
function leafcolor_widget_style_extra_control()
{   
    ...
    // output our extra widget logic field
    echo "
	<p id='ia-".$id_disp."'><label for='".$id_disp."-leafcolor2'>".__('Widget Style', 'sportcenter').": 
