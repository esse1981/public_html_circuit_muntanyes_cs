...
<?php
	...
	<div class="content-image">
			<?php
				preg_match("/<embed\s+(.+?)>/i", $post->post_content, $matches_emb); if(isset($matches_emb[0])){
					echo wp_kses_post($matches_emb[0]);
				}
...
<?php }else {
	...
	if($single_show_image!='1'){
		if(has_post_thumbnail()){
			...
			<div class="content-image"><img src="<?php echo esc_url($thumbnail[0]) ?>" alt="<?php the_title_attribute(); ?>" title="<?php the_title_attribute(); ?>"></div>
		<?php }
	}
}